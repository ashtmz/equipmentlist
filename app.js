const STORAGE_KEYS = {
  ...
  appSettings: "equipment-manager-app-settings",
};
...
function createAppSettings(seed = {}) {
  const pageTitle = String(seed?.pageTitle ?? APP_CONFIG.pageTitle ?? "機材管理ボード").trim();
  return {
    pageTitle: pageTitle || "機材管理ボード",
    estimatePageSettings: createEstimateExportSettings(seed?.estimatePageSettings || {}),
  };
}

function createEstimateExportSettings(seed = {}) {
  return {
    showCostPrice: Boolean(seed?.showCostPrice),
    showStock: Boolean(seed?.showStock),
    ignoreStockLimit: Boolean(seed?.showStock) ? false : Boolean(seed?.ignoreStockLimit),
    showAdjustments: Boolean(seed?.showAdjustments),
  };
}
...
function normalizeEquipment(item, index) {
  ...
  return {
    ...
    isNetPrice: Boolean(item?.isNetPrice),
    priceLower: toPositiveInt(item?.priceLower, 0),
...
function createAddForm(seed = {}) {
  ...
  return {
    ...
    price: seed.price ?? 0,
    notes: seed.notes ?? "",
    isNetPrice: seed.isNetPrice ?? false,
    priceLower: seed.priceLower ?? 0,
...
function loadCachedSharedState() {
  ...
  const appSettings = createAppSettings(loadJSON(STORAGE_KEYS.appSettings, {}));
...
const state = {
  ...
  appSettings: cachedSharedState.appSettings,
...
function applySharedState(rawState) {
  ...
  const hasAppSettings = Object.prototype.hasOwnProperty.call(rawState || {}, "appSettings");
  ...
  const appSettings = hasAppSettings
    ? createAppSettings(rawState?.appSettings)
    : createAppSettings(state.appSettings);
  const maxId = equipment.reduce((max, item) => Math.max(max, toPositiveInt(item.id, 0)), 0);
...
function canApplyRemoteStateImmediately() {
  return !state.showAddModal && !state.showPackModal && !state.showExtraFeeModal && !state.showCheckoutModal && !state.returnModal && !state.showPriceCalcModal && !state.estimateExportModal;
}
...
function buildExportPayload(settingsOverride = null) {
  const estimatePageSettings = createEstimateExportSettings(settingsOverride || state.appSettings?.estimatePageSettings || {});
  return {
    version: 1,
...
function buildLocalMasterSnapshot(settings = {}) {
  const pageTitle = String(settings.pageTitle || "").trim();
  return {
    ...buildExportPayload(),
    appSettings: {
      ...state.appSettings,
      pageTitle: pageTitle || state.appSettings.pageTitle,
    },
  };
}

function buildClientEstimatePageHtml(templateHtml, payload) {
  const serializedPayload = JSON.stringify(payload, null, 2).replaceAll("</script>", "<\\/script>");
  const withPayload = templateHtml.replace(
    /window\.__CLIENT_ESTIMATE_PAYLOAD__ = [\s\S]*?;/,
    `window.__CLIENT_ESTIMATE_PAYLOAD__ = ${serializedPayload};`
  );

  if (withPayload === templateHtml) {
    throw new Error("Client estimate payload marker not found");
  }

  return withPayload;
}

function buildClientEstimateSnapshot(projectId, settings) {
  const project = getEstimateProject(projectId);
  if (!project) {
    return null;
  }

  const packIds = new Set((project.packItems || []).map((entry) => String(entry.packId)).filter(Boolean));
  const equipIds = new Set((project.items || []).map((entry) => Number(entry.equipId)).filter((id) => id > 0));

  state.packs.forEach((pack) => {
    if (!packIds.has(String(pack.id))) {
      return;
    }

    (pack.items || []).forEach((entry) => {
      const equipId = Number(entry.equipId);
      if (equipId > 0) {
  ...
  const equipment = state.equipment
    .filter((item) => equipIds.has(Number(item.id)))
    .map((item) => ({
      id: item.id,
      ...
      genre: item.genre,
      types: item.types,
      stock: settings.showStock ? item.stock : 0,
      price: item.price,
      isNetPrice: settings.showCostPrice ? item.isNetPrice : false,
      priceLower: settings.showCostPrice ? item.priceLower : 0,
    }));

  const packs = state.packs
    .filter((pack) => packIds.has(String(pack.id)))
    .map((pack) => ({
      id: pack.id,
      name: pack.name,
      price: pack.price,
      items: (pack.items || [])
        .filter((entry) => equipIds.has(Number(entry.equipId)))
        .map((entry) => ({
          id: entry.id,
  ...
  const estimateProjects = [{
    ...
    packItems: (project.packItems || []).map((entry) => ({
      id: entry.id,
      packId: entry.packId,
      qty: entry.qty,
    })),
    extraItems: (project.extraItems || []).map((entry) => ({
      id: entry.id,
      name: entry.name,
  ...
  return {
    ...
    dependencyRules: [],
    appSettings: {
      pageTitle: state.appSettings.pageTitle,
    },
  };
}
...
function downloadTextFile(filename, content, mimeType) {
  ...
  if (typeof window.showSaveFilePicker === "function") {
    const acceptMimeType = String(mimeType || "application/octet-stream").split(";")[0] || "application/octet-stream";
    const extensionMatch = String(filename || "").match(/(\.[^.]+)$/);
    const extension = extensionMatch ? extensionMatch[1] : ".txt";

    try {
      const pickerResult = window.showSaveFilePicker({
      ...
      if (pickerResult && typeof pickerResult.then === "function") {
        void pickerResult.then(async (handle) => {
...
function buildEquipmentLabel(item) {
  ...
  return [String(item.manufacturer || "").trim(), String(item.name || "").trim()]
    .filter(Boolean)
    .join(" ");
}

function mergeQtyMapValue(map, key, qty) {
  if (!key || qty <= 0) {
    return;
  }
...
function getSelectionProjectRequirementMap(project) {
  const merged = new Map();

  (project?.items || []).forEach((entry) => {
    mergeQtyMapValue(merged, entry.equipId, toPositiveInt(entry.qty, 0));
  });

  (project?.packItems || []).forEach((entry) => {
    const pack = getPack(entry.packId);
    (pack?.items || []).forEach((packItem) => {
      mergeQtyMapValue(merged, packItem.equipId, toPositiveInt(packItem.qty, 0) * toPositiveInt(entry.qty, 0));
    });
  });
...
function getProjectHistoryEntries(projectName) {
  const normalizedProjectName = String(projectName || "").trim();
  return state.history.filter((item) => String(item.projectName || "").trim() === normalizedProjectName);
}
...
function renderPrintSummary(summaryItems, compact = false) {
  if (!Array.isArray(summaryItems) || summaryItems.length === 0) {
    return "";
  }

  return `
    <div class="print-summary-grid${compact ? " print-summary-grid-compact" : ""}">
...
function renderPrintTable(headers, rows, emptyMessage = "出力対象がありません。") {
      ...
      <tbody>
        ${safeRows.length > 0
          ? safeRows.map((cells) => `
            ...
            `).join("")
          : `<tr><td colspan="${headers.length}" class="print-empty">${escapeHTML(emptyMessage)}</td></tr>`}
      </tbody>
    </table>
  `;
}

function renderPrintSection(title, body, description = "") {
  return `
...
function buildPrintDocument({ title, subtitle = "", sections = [] }) {
  return `<!doctype html>
<html lang="ja">
  <head>
    <meta charset="UTF-8" />
    ...
    <style>
      ...
      * {
      ...
      .print-summary-grid-compact .print-summary-value {
        font-size: 13px;
      }
    ...
    <div class="print-shell">
      <header class="print-header">
        ...
        <div class="print-meta">
          出力日時<br />${escapeHTML(new Date().toLocaleString("ja-JP"))}
        </div>
      </header>
      ${sections.join("")}
    </div>
...
function openPrintDocument({ title, subtitle = "", sections = [] }) {
  ...
  printWindow.document.write(buildPrintDocument({ title, subtitle, sections }));
  printWindow.document.close();
}

function exportEstimatePdf(projectId = state.activeEstimateProjectId) {
  const project = getEstimateProject(projectId);
  if (!project) {
    window.alert("見積もりが見つかりませんでした。");
    return;
  }

  const calc = calcProjectPrice(project.id, []);
  const sections = [
    renderPrintSection(
      "見積サマリー",
      renderPrintSummary([
        { label: "機材明細", value: `${project.items.length} 件` },
        { label: "セット", value: `${(project.packItems || []).length} 件` },
        { label: "追加費用", value: `${(project.extraItems || []).length} 件` },
        { label: "小計", value: formatPrice(calc.subtotal) },
        { label: "合計", value: formatPrice(calc.total) },
      ], true),
      project.name
    ),
    renderPrintSection(
      ...
      renderPrintTable(
        ...
        calc.items.map((entry) => [
          escapeHTML(entry.scopeKey.startsWith("equipment:") ? "機材" : entry.scopeKey.startsWith("pack:") ? "セット" : "追加費用"),
          escapeHTML(entry.name),
          escapeHTML(formatPrice(entry.unitPrice)),
          escapeHTML(String(entry.qty)),
          escapeHTML(formatPrice(entry.lineTotal)),
        ]),
        "見積明細がありません。"
      )
    ),
  ...
  if ((project.packItems || []).length > 0) {
    sections.push(
      renderPrintSection(
        "選択中のセット",
        renderPrintTable(
          ["セット名", "数量", "単価", "内容"],
          project.packItems.map((entry) => {
            const pack = getPack(entry.packId);
            return [
              escapeHTML(pack?.name || "(不明)"),
              escapeHTML(String(entry.qty)),
              escapeHTML(formatPrice(pack?.price || 0)),
              escapeHTML(getPackItemSummary(pack) || "-"),
            ];
          })
        )
      )
  ...
  openPrintDocument({
    title: `見積書 ${project.name}`,
    subtitle: `保存時のファイル名目安: ${sanitizeFileName(`見積書_${project.name}`)}.pdf`,
    sections,
  });
}

function exportSelectionPdf(projectId = state.activeSelectionProjectId) {
  const project = getSelectionProject(projectId);
  if (!project) {
    window.alert("現場が見つかりませんでした。");
    return;
  }

  const requirementMap = getSelectionProjectRequirementMap(project);
  const rows = Array.from(requirementMap.entries())
    .map(([equipId, qty]) => {
      const item = getEquipment(equipId);
      return {
        name: buildEquipmentLabel(item),
        cells: [
          escapeHTML(item?.manufacturer || "-"),
          escapeHTML(item?.name || "(不明)"),
          escapeHTML(formatTypesSummary(item?.genres || []) || "-"),
          escapeHTML(formatTypesSummary(item?.types || []) || "-"),
          escapeHTML(String(qty)),
          escapeHTML(String(item?.stock ?? 0)),
          escapeHTML(String(getAvailable(equipId))),
          escapeHTML(formatOwnershipSummary(item?.ownerships || []) || "-"),
        ],
      };
    })
    .sort((a, b) => compareText(a.name, b.name));
  const warnings = getDependencyWarnings(project.id);
  const historyEntries = getProjectHistoryEntries(project.name);
  const activeCheckoutQty = historyEntries.filter((entry) => !entry.returned).reduce((sum, entry) => sum + entry.qty, 0);
  const sections = [
    renderPrintSection(
      "選定サマリー",
      renderPrintSummary([
        { label: "現場名", value: project.name },
        { label: "選定機材数", value: `${rows.length} 種類` },
        { label: "選定合計", value: `${rows.reduce((sum, entry) => sum + Number(entry.cells[4]), 0)} 点` },
        { label: "持ち出し中", value: `${activeCheckoutQty} 点` },
      ])
    ),
    renderPrintSection(
      "選定表",
      renderPrintTable(
        ["メーカー", "機材名", "ジャンル", "種類", "選定数", "在庫", "現在残数", "所有内訳"],
  ...
  if (warnings.length > 0) {
    sections.push(
      renderPrintSection(
        "不足している関連機材",
        renderPrintTable(
          ["基準機材", "必要機材", "現在数", "必要数", "不足", "区分"],
          warnings.map((warning) => [
            escapeHTML(`${warning.sourceItem?.name || "?"} ×${warning.sourceQty}`),
            escapeHTML(warning.targetItem?.name || "?"),
            escapeHTML(String(warning.currentTargetQty)),
            escapeHTML(String(warning.neededTargetQty)),
            escapeHTML(String(warning.shortfall)),
            escapeHTML(warning.required ? "必須" : "推奨"),
          ])
        ),
        "選定条件に基づく補足です。"
      )
  ...
  openPrintDocument({
    title: `選定表 ${project.name}`,
    subtitle: `保存時のファイル名目安: ${sanitizeFileName(`選定表_${project.name}`)}.pdf`,
    sections,
  });
}

function exportCheckoutPdf(projectId = state.activeSelectionProjectId) {
  const project = getSelectionProject(projectId);
  if (!project) {
    window.alert("現場が見つかりませんでした。");
    return;
  ...
  if (historyEntries.length === 0) {
    window.alert("この現場には持ち出し履歴がありません。");
    return;
  }

  const groups = groupHistoryEntries(historyEntries);
  const sections = [
    renderPrintSection(
      "持ち出しサマリー",
      renderPrintSummary([
        { label: "現場名", value: project.name },
        { label: "持ち出し件数", value: `${groups.length} 件` },
        { label: "持ち出し点数", value: `${historyEntries.reduce((sum, entry) => sum + entry.qty, 0)} 点` },
        { label: "未返却", value: `${historyEntries.filter((entry) => !entry.returned).reduce((sum, entry) => sum + entry.qty, 0)} 点` },
      ])
    ),
  ];
      ...
      renderPrintSection(
        ...
        `
          <div class="print-subsection">
            <span class="print-status">${escapeHTML(allReturned ? "返却済み" : "持ち出し中")}</span>
            <p class="print-inline-note">担当: ${escapeHTML(person || "-")} / 持ち出し先: ${escapeHTML(destination || "-")} / 期間: ${escapeHTML(formatDateRange(startDate, endDate))}</p>
          </div>
          <div class="print-subsection">
            ${renderPrintTable(
              ["機材", "数量", "所有割当", "状態", "返却日"],
              items.map((item) => [
                escapeHTML(buildEquipmentLabel(getEquipment(item.equipId))),
                escapeHTML(String(item.qty)),
                escapeHTML(formatAllocationSummary(item.allocations) || "-"),
                escapeHTML(item.returned ? "返却済み" : "持ち出し中"),
                escapeHTML(item.returnDate || "-"),
              ])
            )}
          </div>
        `,
  ...
  openPrintDocument({
    title: `持ち出し表 ${project.name}`,
    subtitle: `保存時のファイル名目安: ${sanitizeFileName(`持ち出し表_${project.name}`)}.pdf`,
    sections,
  });
}

function exportHistoryGroupPdf(historyIdsRaw) {
  const historyIds = String(historyIdsRaw || "")
    .split(",")
  ...
  if (items.length === 0) {
    window.alert("出力対象の持ち出し履歴が見つかりませんでした。");
    return;
  }

  const first = items[0];
  const allReturned = items.every((item) => item.returned);
  const label = first.projectName || first.destination || first.person || "履歴";

  openPrintDocument({
    title: `持ち出し表 ${label}`,
    subtitle: `保存時のファイル名目安: ${sanitizeFileName(`持ち出し表_${label}`)}.pdf`,
    sections: [
      renderPrintSection(
        "持ち出しサマリー",
        renderPrintSummary([
          { label: "現場名", value: first.projectName || "-" },
          { label: "担当", value: first.person || "-" },
          { label: "持ち出し先", value: first.destination || "-" },
          { label: "状態", value: allReturned ? "返却済み" : "持ち出し中" },
          { label: "期間", value: formatDateRange(first.startDate || first.date, first.endDate || first.date) },
          { label: "合計", value: `${items.reduce((sum, item) => sum + item.qty, 0)} 点` },
        ])
      ),
      renderPrintSection(
        "持ち出し明細",
...
function exportOverallResultPdf() {
  const aggregate = new Map();

  state.selectionProjects.forEach((project) => {
    getSelectionProjectRequirementMap(project).forEach((qty, equipId) => {
      const current = aggregate.get(equipId) ?? { qty: 0, projects: [] };
      current.qty += qty;
      current.projects.push(`${project.name} ×${qty}`);
      aggregate.set(equipId, current);
    });
  });

  if (aggregate.size === 0 && state.history.length === 0) {
    window.alert("全体リザルトに出力できるデータがありません。");
    return;
  }
    ...
    .map(([equipId, entry]) => {
      ...
      return {
        ...
        cells: [
          ...
          escapeHTML(String(item?.stock ?? 0)),
          escapeHTML(String(entry.qty)),
          escapeHTML(String(getCheckedOutQty(equipId))),
          escapeHTML(String(getAvailable(equipId))),
          escapeHTML(entry.projects.join(" / ")),
        ],
      };
    })
    .sort((a, b) => compareText(a.name, b.name));
  ...
  const sections = [
    renderPrintSection(
      "全体サマリー",
      renderPrintSummary([
        { label: "現場数", value: `${state.selectionProjects.length} 件` },
        { label: "選定合計", value: `${aggregateRows.reduce((sum, entry) => sum + Number(entry.cells[3]), 0)} 点` },
        { label: "持ち出し中", value: `${state.history.filter((entry) => !entry.returned).reduce((sum, entry) => sum + entry.qty, 0)} 点` },
        { label: "機材種類", value: `${aggregateRows.length} 種類` },
      ])
    ),
    renderPrintSection(
      "全体リザルト",
      renderPrintTable(
        ["メーカー", "機材名", "在庫", "選定合計", "持ち出し中", "現在残数", "現場内訳"],
        aggregateRows.map((entry) => entry.cells),
        "集計対象の選定データがありません。"
      ),
      "全現場の選定状況を合算した一覧です。"
    ),
  ...
  state.selectionProjects.forEach((project) => {
    const projectRows = Array.from(getSelectionProjectRequirementMap(project).entries())
      .map(([equipId, qty]) => {
        const item = getEquipment(equipId);
        return {
          name: buildEquipmentLabel(item),
      ...
      .sort((a, b) => compareText(a.name, b.name));
    const projectHistory = getProjectHistoryEntries(project.name);
    sections.push(
      renderPrintSection(
        `現場別リザルト: ${project.name}`,
        `
          ${renderPrintSummary([
            { label: "選定種類", value: `${projectRows.length} 種類` },
            { label: "選定合計", value: `${projectRows.reduce((sum, entry) => sum + Number(entry.cells[2]), 0)} 点` },
            { label: "持ち出し履歴", value: `${projectHistory.length} 件` },
            { label: "未返却", value: `${projectHistory.filter((entry) => !entry.returned).reduce((sum, entry) => sum + entry.qty, 0)} 点` },
          ])}
          <div class="print-subsection">
            ${renderPrintTable(
              ["メーカー", "機材名", "選定数", "持ち出し中", "現在残数"],
              projectRows.map((entry) => entry.cells),
              "この現場の選定はありません。"
            )}
          </div>
        `
  ...
  const activeHistory = state.history.filter((entry) => !entry.returned);
  if (activeHistory.length > 0) {
    sections.push(
      renderPrintSection(
        "持ち出し中一覧",
        renderPrintTable(
          ["現場", "機材", "数量", "持ち出し先", "担当", "期間", "所有割当"],
          activeHistory.map((entry) => [
            escapeHTML(entry.projectName || "-"),
            escapeHTML(buildEquipmentLabel(getEquipment(entry.equipId))),
            escapeHTML(String(entry.qty)),
            escapeHTML(entry.destination || "-"),
            escapeHTML(entry.person || "-"),
            escapeHTML(formatDateRange(entry.startDate || entry.date, entry.endDate || entry.date)),
            escapeHTML(formatAllocationSummary(entry.allocations) || "-"),
          ])
        )
      )
    );
  ...
  openPrintDocument({
    title: "全体リザルト",
    subtitle: `保存時のファイル名目安: ${sanitizeFileName("全体リザルト")}.pdf`,
    sections,
  });
}

function exportAppData() {
  state.estimateExportModal = {
    settings: createEstimateExportSettings(state.appSettings?.estimatePageSettings || {}),
  };
  render();
}

function downloadExportDataJson(settingsOverride = null) {
  const stamp = new Date().toISOString().slice(0, 10);
  const payload = buildExportPayload(settingsOverride);
  downloadTextFile(`equipment-export-${stamp}.json`, JSON.stringify(payload, null, 2), "application/json");
}

function closeEstimateExportModal() {
  state.estimateExportModal = null;
  render();
  refreshRemoteStateIfPending();
}

function syncEstimateExportSettingsFromDOM() {
  const root = document.querySelector("[data-estimate-export-form]");
  if (!root || !state.estimateExportModal) {
    return;
  }

  const showStock = root.querySelector("#estimate-export-show-stock")?.checked ?? false;
  state.estimateExportModal.settings = createEstimateExportSettings({
    showCostPrice: root.querySelector("#estimate-export-show-cost-price")?.checked,
    showStock,
    ignoreStockLimit: showStock
      ? false
      : root.querySelector("#estimate-export-ignore-stock-limit")?.checked,
    showAdjustments: root.querySelector("#estimate-export-show-adjustments")?.checked,
  });
}
...
function parseImportedData(rawText) {
  ...
  const appSettings = createAppSettings(parsed.appSettings);
  const maxId = equipment.reduce((max, item) => Math.max(max, toPositiveInt(item.id, 0)), 0);
...
async function importAppData(file) {
  ...
  try {
    ...
    state.appSettings = imported.appSettings || createAppSettings();
...
function groupHistory() {
  return groupHistoryEntries(state.history);
}
...
function syncAddFormFromDOM() {
  ...
  const isNetPrice = root.querySelector("#add-net-price")?.checked ?? false;
  ...
  state.addForm = {
    ...
    isNetPrice,
    priceLower: toPositiveInt(root.querySelector("#add-price-lower")?.value, state.addForm.priceLower ?? 0),
...
function saveEquipmentFromForm() {
  ...
  const nextItem = {
    ...
    isNetPrice: Boolean(state.addForm.isNetPrice),
    priceLower: Boolean(state.addForm.isNetPrice) ? toPositiveInt(state.addForm.priceLower, 0) : 0,
            ...
            return `
              <article class="inventory-card">
                ...
                <div class="inventory-card-bottom">
                  ...
                  ${(item.price > 0 || (item.isNetPrice && item.priceLower > 0)) ? `
                  <div class="info-group inventory-price">
                    <span class="info-label">${item.isNetPrice ? "上代 / 下代" : "価格"}</span>
                    <p class="note-text">${escapeHTML(formatPrice(item.price))}${item.isNetPrice ? ` / ${escapeHTML(formatPrice(item.priceLower))}` : ""}</p>
                  </div>
                  ` : ""}
      ...
      <section class="panel">
        <div class="section-head">
          ...
          <div class="section-head-actions">
            <button class="mini-button" data-action="export-overall-result-pdf" ${(state.selectionProjects.length === 0 && state.history.length === 0) ? "disabled" : ""}>全体リスト</button>
      ...
      <section class="panel">
        <div class="section-head">
          ...
          <div class="selection-workspace-actions">
            ...
            <button class="ghost-button" data-action="export-selection-pdf" data-project-id="${escapeHTML(project.id)}" ${project.items.length === 0 ? "disabled" : ""}>選定表PDF</button>
              ...
              <article class="history-card ${allReturned ? "done" : "active"}">
                <header class="history-header">
                  ...
                  <div class="history-header-actions">
                    ...
                    <button class="mini-button" data-action="export-history-group-pdf" data-history-ids="${escapeHTML(groupHistoryIds)}">持ち出し表PDF</button>
      ...
      <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="add-modal-title">
        ...
        <div data-add-form>
          <div class="form-grid">
            ...
            <div class="field">
              <label class="checkbox-label" for="add-net-price">
                <input id="add-net-price" type="checkbox" ${state.addForm.isNetPrice ? "checked" : ""} />
                <span>NET</span>
              </label>
            </div>
          </div>
          ...
          <div class="form-grid">
            <div class="field">
              <label for="add-price">${state.addForm.isNetPrice ? "上代" : "価格"}</label>
              <input id="add-price" class="number-input" type="number" min="0" value="${toPositiveInt(state.addForm.price, 0)}" placeholder="0" />
            </div>
            ${state.addForm.isNetPrice ? `
              <div class="field">
                <label for="add-price-lower">下代</label>
                <input id="add-price-lower" class="number-input" type="number" min="0" value="${toPositiveInt(state.addForm.priceLower, 0)}" placeholder="0" />
              </div>
            ` : ""}
          </div>
    ...
    <section class="panel">
      <div class="section-head">
        ...
        <div class="section-head-actions">
          ...
          <button class="ghost-button" data-action="convert-estimate-to-selection" data-project-id="${escapeHTML(project?.id || "")}" ${project ? "" : "disabled"}>選定に反映</button>
        </div>
      </div>
                  ...
                  <div class="inventory-card-top">
                    <div class="inventory-card-main">
                      ...
                      <div class="inventory-inline-groups">
                        ...
                        <div class="inline-info">
                          <span class="info-label">${item.isNetPrice ? "上代" : "価格"}</span>
                          <span class="price-value">${escapeHTML(formatPrice(item.price))}</span>
                          ${item.isNetPrice ? `<span class="sub-note">下代 ${escapeHTML(formatPrice(item.priceLower))}</span>` : ""}
                        </div>
                        <div class="inline-info">
  ...
  return `
    <div class="modal open">
      ...
      <div class="modal-card modal-card-wide" role="dialog" aria-modal="true" aria-labelledby="price-calc-modal-title">
        ...
        <div class="modal-actions">
          <button class="ghost-button" data-action="export-estimate-pdf" data-project-id="${escapeHTML(project.id)}">見積PDF</button>
...
function renderEstimateExportModal() {
  if (!state.estimateExportModal) {
    return "";
  }

  const settings = state.estimateExportModal.settings;
  return `
    <div class="modal open">
      <div class="modal-backdrop" data-action="close-estimate-export-modal"></div>
      <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="estimate-export-modal-title">
        <div class="modal-head">
          <div>
            <h2 id="estimate-export-modal-title">書き出し設定</h2>
          </div>
          <button class="modal-close" data-action="close-estimate-export-modal" aria-label="閉じる">×</button>
        </div>

        <div data-estimate-export-form>
          <div class="ownership-editor">
            <div class="form-grid">
              <label class="field field-checkbox"><input id="estimate-export-show-cost-price" type="checkbox" ${settings.showCostPrice ? "checked" : ""} /> 仕入れ値（下代）も表示する</label>
              <label class="field field-checkbox"><input id="estimate-export-show-stock" type="checkbox" ${settings.showStock ? "checked" : ""} /> 在庫数を表示する</label>
              <label class="field field-checkbox"><input id="estimate-export-ignore-stock-limit" type="checkbox" ${settings.ignoreStockLimit ? "checked" : ""} ${settings.showStock ? "disabled" : ""} /> 数量追加で在庫上限を無視する</label>
              <label class="field field-checkbox"><input id="estimate-export-show-adjustments" type="checkbox" ${settings.showAdjustments ? "checked" : ""} /> 割引・割増設定を表示する</label>
            </div>
          </div>
        </div>

        <div class="modal-actions">
          <button class="ghost-button" data-action="close-estimate-export-modal">キャンセル</button>
          <button class="primary-button" data-action="confirm-estimate-export">JSONを書き出す</button>
        </div>
      </div>
    </div>
  `;
...
function applyPageTitle() {
  const title = state.appSettings?.pageTitle || "機材管理ボード";
...
function editPageTitle() {
  const currentTitle = state.appSettings?.pageTitle || "機材管理ボード";
  const nextTitle = window.prompt("ページタイトルを入力してください。", currentTitle);
  if (nextTitle === null) {
    return;
  }
  ...
  if (!trimmedTitle) {
    window.alert("ページタイトルを空欄にはできません。");
    return;
  }

  state.appSettings = createAppSettings({
    ...state.appSettings,
...
function render() {
  ...
  app.innerHTML = `
    <header class="hero">
      <div class="hero-title-wrap">
        <div>
          <h1>${escapeHTML(state.appSettings.pageTitle)}</h1>
          <p class="sync-status ${getSyncStatusClass()}">${escapeHTML(getSyncStatusLabel())}</p>
        </div>
      </div>
      <div class="hero-actions">
        <div class="hero-corner-actions">
          <button class="mini-button" data-action="edit-page-title">タイトル設定</button>
  ...
  ${state.tab === "estimate" ? renderEstimates() : ""}
    ...
    ${renderEstimateExportModal()}
  `;

  applyPageTitle();

  const hasOpenModal = state.showAddModal || state.showPackModal || state.showExtraFeeModal || state.showCheckoutModal || state.showProjectModal || Boolean(state.returnModal) || Boolean(state.mediaViewer) || state.showPriceCalcModal || Boolean(state.estimateExportModal);
...
app.addEventListener("click", (event) => {
  ...
  switch (target.dataset.action) {
    ...
    case "close-project-modal":
      ...
      refreshRemoteStateIfPending();
      return;
    case "close-estimate-export-modal":
      closeEstimateExportModal();
      return;
    ...
    case "export-estimate-pdf":
      ...
      return;
    case "confirm-estimate-export":
      syncEstimateExportSettingsFromDOM();
      state.appSettings = createAppSettings({
        ...state.appSettings,
        estimatePageSettings: state.estimateExportModal?.settings,
      });
      persistState();
      downloadExportDataJson(state.estimateExportModal?.settings);
      closeEstimateExportModal();
      return;
    case "export-selection-pdf":
      exportSelectionPdf(target.dataset.projectId);
      return;
    case "export-checkout-pdf":
      exportCheckoutPdf(target.dataset.projectId);
      return;
    case "export-history-group-pdf":
      exportHistoryGroupPdf(target.dataset.historyIds);
      return;
    case "export-overall-result-pdf":
      exportOverallResultPdf();
      return;
    case "import-data":
      openImportPicker();
      return;
...
app.addEventListener("change", (event) => {
  if (event.target.id === "add-net-price") {
    syncAddFormFromDOM();
    render();
    return;
  }

  if (event.target.id === "estimate-export-show-cost-price" || event.target.id === "estimate-export-show-stock" || event.target.id === "estimate-export-ignore-stock-limit" || event.target.id === "estimate-export-show-adjustments") {
    syncEstimateExportSettingsFromDOM();
    render();
    return;
  }
